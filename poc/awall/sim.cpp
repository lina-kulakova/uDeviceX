...
double xxn[MAX_NP], yyn[MAX_NP], zzn[MAX_NP]; /* "next" particles
						 position */
...
void init_vars() {
  ...
  vx0 = -2; vy0 = 0; vz0 = 0; /* velocity of the particles */
...
double wrp(double r, double c, double L) {
  auto dr = r - c;
...
void upd() {
  ...
  bounce(); /* bouncing of the face */
  upd_pos(); /* [xyz] = [xyz]n */
  pbc();     /* apply periodic BC */
